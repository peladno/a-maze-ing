...
- 6 つのテストファイルと 135 本のテストが、それぞれ何を守っているか
- 補助関数の役目
...
| `test_config.py` | 43 | `maze/config.py`(2 章) |
...
| `_as_int` | 数を受け付ける、数でないものは拒否、最小値未満は拒否、**最小値 1 なら `must be a positive integer`・それ以外なら `must be at least N`**(メッセージ全体を比べる)、**最小値 0 でも範囲を確かめる**、最小値なしなら負も受け付ける |
